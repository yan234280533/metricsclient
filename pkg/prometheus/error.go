...
func (ec *QueryErrorCollector) As(target interface{}) bool {
	if target == nil {
		klog.Errorf("ErrorCollection.As() target cannot be nil")
		return false
	}
	...
	if typ.Kind() != reflect.Ptr || val.IsNil() {
		klog.Errorf("ErrorCollection.As() target must be a non-nil pointer")
		return false
	}
	if e := typ.Elem(); e.Kind() != reflect.Interface && !e.Implements(errorType) {
		klog.Errorf("ErrorCollection.As() *target must be interface or implement error")
		return false
	}
